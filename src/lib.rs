...
pub struct Args {
    ...
    /// the real video root is taken to be the current directory.
    ...
    /// Subtitles are hard-coded if available.
    #[clap(long, conflicts_with_all = &["av1", "x265", "anime", "anime-slow-well-lit", "anime-mixed-dark-battle"])]
    pub for_tv: bool,
}
...
async fn encode_video(
    ...
    if let Some(ref log_path) = input.log_path {
        let mut ffreport = OsString::from("append=1:file=");
        // ':' and '\' must be escaped:
...
async fn convert_ass(sub_path: &PathBuf) -> Result<PathBuf> {
    ...
    dump_stream(&sub_path, &converted_sub_path).await?;
    Ok(converted_sub_path)
}

async fn dump_stream(input_path: &Path, output_path: &Path) -> Result<()> {
