...
fn test_fill_output_template() {
    assert_paths_eq!(
        InputFile::fill_output_template("{basename}-{preset}-{crf}", PathBuf::from("dir"), "foo", "preset", "crf", "avi"),
        "dir/foo-preset-crf.avi");
    assert_paths_eq!(
        InputFile::fill_output_template("{basename}", PathBuf::from("dir"), "foo", "preset", "crf", "avi"),
        "dir/foo.avi");
}

#[tokio::test]
async fn test_output_directory() {
    let args = Arc::new(Args::parse_from(["prog_name", "--output-directory", "encoded"]));
    let input = InputFile::new(Path::new("a/b/vid.en.MP4"), args).await.unwrap();
    assert_paths_eq!(
    ...
    for encode_directory in encode_directories {
        for input_path in input_paths {
            let args = Arc::new(Args::parse_from(
                ["prog_name", "--output-directory", "encoded", encode_directory]));
...
async fn test_absolute_output_directory() {
    let args = Arc::new(Args::parse_from(["prog_name", "--output-directory", "/home/x/encoded"]));
...
async fn test_input_dir_and_absolute_output_dir() {
    let args = Arc::new(Args::parse_from(["prog_name", "--output-directory", "/home/x/encoded", "./a/b"]));
...
async fn test_output_fname() {
    ...
    assert_paths_eq!(
        input.log_path.unwrap(),
        "a/b/encoded/vid.en.MP4.log");

    let args = Arc::new(Args::parse_from(["prog_name", "--x265", "--no-log", "a/b"]));
    let input = InputFile::new(Path::new("a/b/subdir/vid.mp4"), args.clone()).await.unwrap();
    assert_paths_eq!(
        input.get_output_path(None).unwrap(),
        "a/b/encoded/subdir/vid-crf22.mp4");
    assert_eq!(input.log_path, None);

    let input = InputFile::new(Path::new("a/b/vid.MKV"), args.clone()).await.unwrap();
    assert_paths_eq!(
        input.get_output_path(None).unwrap(),
        "a/b/encoded/vid-crf22.mkv");

    let input = InputFile::new(Path::new("outside-root/vid.mkv"), args.clone()).await.unwrap();
    assert!(matches!(input.get_output_path(None), Err(_)));

    let args = Arc::new(Args::parse_from(["prog_name", "--x265", "--no-log", "/a"]));
    let input = InputFile::new(Path::new("/a/vid.flv"), args.clone()).await.unwrap();
    assert_paths_eq!(
        input.get_output_path(None).unwrap(),
